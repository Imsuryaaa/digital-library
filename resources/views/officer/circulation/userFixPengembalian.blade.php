...
<div class="bg-gray-100 border border-gray-300 rounded-lg shadow-md mb-4">
    <div class="p-6">
        ...
        <form action="/indexTransaksiPengembalianBukuUnitCreate/{{$id_detail_peminjaman->id_detail_peminjaman}}" method="post">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    @csrf
                    <label for="nama" class="block mb-1">Unit Book ID:</label>
                    <input type="text" id="nama" name="nama" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" disabled value="{{$id_detail_peminjaman->id_detail_peminjaman}}">
                </div>
                <input type="hidden" name="tgl_dikembalikan" value="{{date('Y-m-d')}}">
                <input type="text" name="id_buku_satuan" value="{{$data[0]->id_buku_satuan}}">
