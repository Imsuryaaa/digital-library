...
{{-- Card 1 Transaksi --}}
    @if ($data->isEmpty())
        ...
        <p class="text-sm flex justify-center">or the unit book id you are looking for does not exist</p>
        @if (Auth::user()->level == 'admin')       
            <a href="/admin" class="bg-[#A63F0F] flex justify-center btn btn-sm hover:bg-[#174E71] text-white font-bold py-2 px-4 rounded" type="button"><i class="bi bi-arrow-90deg-left"></i> Back</a>
        @else
            <a href="/petugas" class="bg-[#A63F0F] flex justify-center btn btn-sm hover:bg-[#174E71] text-white font-bold py-2 px-4 rounded" type="button"><i class="bi bi-arrow-90deg-left"></i> Back</a>
        @endif
    @else
    ...
    <div class="bg-gray-100 border border-gray-300 rounded-lg shadow-md mb-4">
        <div class="p-6">
            @if (Auth::user()->level == 'admin')
            <a href="/pilihUserPeengembalian" class="btn bg-[#A63F0F] hover:bg-[#174E71] text-white btn-sm"><i class="bi bi-arrow-90deg-left"></i> Back</a>    
            @else
            <a href="/officer/pilihUserPeengembalian" class="btn bg-[#A63F0F] hover:bg-[#174E71] text-white btn-sm"><i class="bi bi-arrow-90deg-left"></i> Back</a>
            @endif
            <input type="text" name="id_user" value="{{$data[0]->id}}" hidden id="">
            ...
            @else       
            <form action="/officer/transaksiUserPengembalian/{{$data[0]->id}}" method="GET">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        @csrf
                        <label for="nama" class="block mb-1">Full Name:</label>
                    ...
                    @foreach ($data as $item)
                        <tr class="text-center">
                            ...
                            <td class="border px-4 py-2">
                                ...
                                @if (Auth::user()->level == 'admin')
                                <a href="{{ '/indexTransaksiPengembalianBukuUnit/' . $item->id . '/' . $item->id_detail_peminjaman }}" class="btn bg-[#85c6ee] btn-xs text-black rounded-md hover:bg-[#5d98bc]">Process now!</a>
                                @else
                                <a href="{{ '/indexTransaksiPengembalianBukuUnit/' . $item->id . '/' . $item->id_detail_peminjaman }}" class="btn bg-[#85c6ee] btn-xs text-black rounded-md hover:bg-[#5d98bc]">Process now!</a>
                                @endif 
                                
                                @else
                                    <p class="font-bold text-green-500">DONE</p>
                                @endif
                            </td>
                        </tr>
                    @endforeach
